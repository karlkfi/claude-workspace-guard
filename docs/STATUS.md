...
| <a id="Q17"></a>Q17 | Conditionally emit `deny` in headless / full-auto runs | `security` `docs` | 🔲 | S | README's Limitations claims `ask` "effectively blocks" headless, but Claude Code docs say a hook's `ask`/`defer` is remapped to `allow` there (fail-open, violating secure-by-default). Verify the headless behavior end-to-end first; if confirmed, emit `deny` when non-interactive (keep `ask` interactive) and correct the README Limitation. |
