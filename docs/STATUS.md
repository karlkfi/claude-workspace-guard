...
|---|---|---|---|---|---|
| <a id="Q18"></a>Q18 | Newlines swallowed by shlex merge adjacent commands into one group | `parsing` `security` `bug` | 🔲 | S | `shlex` treats `\n` as whitespace so a command on the line after another (no `;`) merges into the prior group: false positives (extra tokens read as file args) and a false negative — a guarded command after an unguarded one escapes the guard. Fix: emit newlines as separator tokens (already listed in `SEPARATORS`); add multi-line fixtures. |
