...
|---|---|---|---|---|---|
| <a id="Q19"></a>Q19 | Plugin wiring untested (hooks.json / plugin.json) | `tests` `infra` | 🔲 | S | No test asserts the config plumbing that connects the script to Claude Code is valid: `hooks/hooks.json` is well-formed JSON, registers `PreToolUse`/`Bash`, and its `command` references a script path that exists; `.claude-plugin/{plugin,marketplace}.json` parse. Catches the "281 tests green but plugin silently doesn't load" failure. Stdlib `json`/`os` only. |
