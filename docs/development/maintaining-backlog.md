...
`docs/STATUS.md` is the single source of truth for project progress and priorities. Its format and maintenance process are defined by the globally-installed **`session-backlog` skill** — invoke it for any change to the Queue or header rather than following rules copied here; a local copy would drift.
