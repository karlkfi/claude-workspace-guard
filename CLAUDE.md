...
- Every PR body ends with the `## Release note` block from `.github/pull_request_template.md`: one line in release-bullet voice when a decision moves or an operator-visible message changes, else `None` (meaning "no bullet — fold into the changelog link"). Leaving it empty is not `None`; it reports as unanswered when the release notes are drafted from these. `gh pr create --body` does **not** apply the template, so copy the block in yourself.
